...
OIDin_R1="` snmpwalk -c oss-router -v 1 $RJ1oid | grep "$i"-IN | awk '{print $1}' |  sed -e 's/^.\{29\}//' `"
OIDout_R1="`snmpwalk -c oss-router -v 1 $RJ1oid | grep "$i"-OUT | awk '{print $1}' |  sed -e 's/^.\{29\}//' `" 
...
OIDin_R2="` snmpwalk -c oss-router -v 1 $RJ2oid | grep "$i"-IN | awk '{print $1}' |  sed -e 's/^.\{29\}//' `"
OIDout_R2="`snmpwalk -c oss-router -v 1 $RJ2oid | grep "$i"-OUT | awk '{print $1}' |  sed -e 's/^.\{29\}//' `"
...
done < /usr/traffic-counter/counters/modem_sn.txt
